...
if($ENV{REQUEST_METHOD} eq 'GET') {
    ...
    if($ENV{REMOTE_USER}) {
        $userstuff = <<1HERE1You are logged in as <strong>$ENV{REMOTE_USER}</strong>.
1HERE1    ;
    } else {
        $userstuff = <<1HERE1
Your Name<br>
...
<input type="text" name="subject" value="" size=50><br>
<small>(bad subjects='Check This Out!' or 'An Article'. If yours isn't clear, it'll be deleted!)</small>
<p>
...
1HERE1
    ...
    if(open IN, "<" . $engine->filename($engine->url('@@NEWS@@/',
        ...
        foreach (sort keys %types) {
            $engine->body_push("<option value=\"$types{$_}\">$_</option>");
        }
    }
...
<p>
The Scoop (HTML is fine, but double check the URLS and Tags! Remember to put
&lt;p&gt; between paragraphs. An ampersand (&amp;) is `&amp;amp;', less-than
(&lt;) is `&amp;lt;', and greater-than (&gt;) is `&amp;gt;')<br>
<textarea name="story" wrap=virtual rows=15 cols=50></textarea><br>
...
1HERE1
    ...
    my $filename = time() . ".$$." . $engine->argument('category') . ".thtml";
    ...
    if($engine->argument('email') =~ /\@/) {
        $newsheaders{'Link'} = "<a href=\"mailto:" . $engine->argument('email')
    ...
    $newsbody = scalar($engine->argument('story'));
    $newsbody =~ tr/\r\n/  /;
    $newsbody =~ s/(.{0,60}[^\s]*)/$1\n/g;
    $newsbody =~ s/<p>([^\n])/<p>\n$1/g;
    $newsbody =~ s/([^\n])<p>/$1\n<p>/g;
    ...
    if($ENV{REMOTE_USER} && ($ENV{REMOTE_USER} !~ /unknown/i)) {
        ...
        $newsheaders{'Author'} = $person->{'familiarName'} || $ENV{REMOTE_USER};
        ...
        if($$person{'mail'} !~ /^\s*$/) {
            ...
            $newsheaders{Link} = $$person{familiarName} || $ENV{REMOTE_USER};
        }
